...
//Probability of player A loses / B wins
...
void update(double K, double K2) {
	for(int i=0;i<7;i++) 
		users[i].new_rating=users[i].old_rating+users[i].delta*K2/K;
...
int main() {
	...
	//outAMSE << "K" << "," << "AMSE" << endl;
	ofstream outAMSE2("tests/AMSE2.csv", ios::out | ios::trunc);
	outAMSE2 << "K2" << "," << "AMSE" << endl;
	ofstream outMSE("tests/MSE.csv", ios::out | ios::trunc);
	outMSE << "Index" << "," << "MSE" << endl;

	//for(double K=1.3;K<=1.5001;K+=0.01) {
	double K=1.44;
	for(double K2=0.7;K2<=0.901;K2+=0.01) {
		aveMSE=0;
		for(int i=0;i<20;i++) {
			users.clear();
		...
		outAMSE2 << K2 << "," << aveMSE/20 << endl;
		printf("K2=%.2f, AveMinSqErr value: %.8f\n",K2,aveMSE/20);
	}
	outAMSE2.close();
	return 0;
}